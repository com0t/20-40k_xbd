 ...
 * и самого Clearfy.
 *
 * $.wbcr_factory_templates_103.app - методы для работы с приложением. Скрыть, показать уведомления.
 * $.wbcr_factory_templates_103.hooks - это иммитация хуков и фильтров аналогично тем, что используются в Wordpress
 *
...
(function($) {
	...
	if( !$.wbcr_factory_templates_103 ) {
		$.wbcr_factory_templates_103 = {};
	}

	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_templates_103.filters = $.wbcr_factory_templates_103.filters || $.wfactory_446.filters;
	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_templates_103.hooks = $.wbcr_factory_templates_103.hooks || $.wfactory_446.hooks;

	$.wbcr_factory_templates_103.app = $.wbcr_factory_templates_103.app || {
		/**
		...
		showNotice: function(message, type) {
			...
			$([document.documentElement, document.body]).animate({
				 ...
				 * Пример регистрации хука $.wfactory_446.hooks.add('wbcr/factory_templates_103/updated',
				 * function(noticeId) {});
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_446.hooks.run('wbcr/factory_templates_103/showed_notice', [noticeId]);
		...
		hideNotice: function(noticeId) {
			...
			el.fadeOut(500, function(e) {
				 ...
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_446.hooks.run('wbcr/factory_templates_103/hidded_notice', [noticeId]);
	...
	$.wfactory_446.hooks.add('core/components/activation_error', function(plugin, button, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_templates_103) ) {
			return false;
		}
		...
		if( response.data && response.data.error_message ) {
			$.wbcr_factory_templates_103.app.showNotice(response.data.error_message, 'danger');
		}
	});
	...
	$.wfactory_446.hooks.add('core/components/ajax_error', function(xhr, ajaxOptions, thrownError) {
		...
		$.wbcr_factory_templates_103.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
	});
