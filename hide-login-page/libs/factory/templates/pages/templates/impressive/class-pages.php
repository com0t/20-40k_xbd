 ...
 */
class PageBase extends \WBCR\Factory_Templates_103\Impressive {
	...
	public function assets($scripts, $styles)
	{
		...
		$this->styles->add(FACTORY_TEMPLATES_103_URL . '/assets/css/clearfy-base.css');
		...
		if( !($this->plugin->has_premium() && $this->plugin->premium->is_active()) ) {
			$this->scripts->add(FACTORY_TEMPLATES_103_URL . '/assets/js/clearfy-widgets.js', [
				'jquery',
				'wfactory-446-core-general',
				'wbcr-factory-templates-103-global'
			], 'wbcr-factory-templates-103-widgets');
		}

		// Script for search form on plugin options
		if( $this->show_search_options_form ) {
			$this->styles->add(FACTORY_TEMPLATES_103_URL . '/assets/css/libs/autocomplete.css');

			$this->scripts->add(FACTORY_TEMPLATES_103_URL . '/assets/js/libs/jquery.autocomplete.min.js');
			$this->scripts->add(FACTORY_TEMPLATES_103_URL . '/assets/js/clearfy-search-options.js');
		}
	...
	{
		wbcr_factory_templates_103_subscribe($this->plugin);
	}
	...
	public function showBusinessSuggetionWidget()
	{
		...
		$default_features = [
			'4_premium' => __('4 premium components now;', 'wbcr_factory_templates_103'),
			'40_premium' => __('40 new premium components within a year for the single price;', 'wbcr_factory_templates_103'),
			'multisite_support' => __('Multisite support;', 'wbcr_factory_templates_103'),
			'advance_settings' => __('Advanced settings;', 'wbcr_factory_templates_103'),
			'no_ads' => __('No ads;', 'wbcr_factory_templates_103'),
			'perfect_support' => __('Perfect support.', 'wbcr_factory_templates_103')
		];

		/**
		 * @since 2.0.8 - added
		 */
		$suggetion_title = __('MORE IN CLEARFY <span>BUSINESS</span>', 'wbcr_factory_templates_103');
		...
		?>
		<div class="wbcr-factory-sidebar-widget wbcr-factory-templates-103-pro-suggettion">
			...
			<a href="<?php echo $purchase_url ?>" class="wbcr-factory-purchase-premium" target="_blank"
			   rel="noopener">
				<?php printf(__('Upgrade for $%s', 'wbcr_factory_templates_103'), $upgrade_price) ?>
			</a>
		</div>
		<?php
	}
		...
		<div class="wbcr-factory-sidebar-widget">
			<ul>
				<li>
					...
					- <?php _e('A neutral setting that can not harm your site, but you must be sure that you need to use it.', 'wbcr_factory_templates_103'); ?>
				</li>
				<li>
					...
					- <?php _e('When set this option, you must be careful. Plugins and themes may depend on this function. You must be sure that you can disable this feature for the site.', 'wbcr_factory_templates_103'); ?>
				</li>
				<li>
					...
					- <?php _e('Absolutely safe setting, We recommend to use.', 'wbcr_factory_templates_103'); ?>
				</li>
			</ul>
			----------<br>
			<p><?php _e('Hover to the icon to get help for the feature you selected.', 'wbcr_factory_templates_103'); ?></p>
		</div>
		<?php
	}
	...
	public function showDonateWidget()
	{
		...
		<div class="wbcr-factory-sidebar-widget">
			<p>
				<strong><?php _e('Donation for plugin development', 'wbcr_factory_templates_103'); ?></strong>
			</p>
			<?php if( get_locale() !== 'ru_RU' ): ?>
				<form id="wbcr-factory-paypal-donation-form" action="https://www.paypal.com/cgi-bin/webscr"
					...
					<input type="image" src="<?php echo FACTORY_TEMPLATES_103_URL ?>/templates/assets/img/paypal-donate.png"
	...
	public function showSubscribeWidget()
	{
		...
		<div id="wbcr-clr-subscribe-widget" class="wbcr-factory-sidebar-widget wbcr-factory-subscribe-widget">
			<p><strong><?php _e('Subscribe to plugin’s newsletter', 'wbcr_factory_templates_103'); ?></strong></p>
			<div class="wbcr-clr-subscribe-widget-body">
				...
				<div class="wbcr-factory-subscribe-widget__message-contanier">
					<div class="wbcr-factory-subscribe-widget__text wbcr-factory-subscribe-widget__text--success">
						<?php _e("Thank you, you have successfully subscribed!", 'wbcr_factory_templates_103') ?>
					</div>
					<div class="wbcr-factory-subscribe-widget__text wbcr-factory-subscribe-widget__text--success2">
						<?php _e("Thank you for your subscription, a confirmation email has been sent to you. You need to confirm your subscription to complete this process.", 'wbcr_factory_templates_103'); ?>
					</div>
				</div>
				...
				<form id="wbcr-factory-subscribe-widget__subscribe-form" method="post" data-nonce="<?php echo wp_create_nonce('clearfy_subscribe_for_' . $this->plugin->getPluginName()) ?>">
					<input id="wbcr-factory-subscribe-widget__email" class="wbcr-factory-subscribe-widget__field" type="email" name="email" placeholder="<?php _e('Enter your email address', 'wbcr_factory_templates_103'); ?>" required>
					<label class="wbcr-factory-subscribe-widget__checkbox-label">
						...
						<?php echo sprintf(__("I confirm to subscribe to the CreativeMotion newsletter to receive the latest news. You can find how we use your information on our %s Privacy Policy %s", 'wbcr_factory_templates_103'), '<a href="' . $terms . '" target="_blank">', '</a>'); ?>
					</label>
					...
					<input type="submit" class="btn wbcr-factory-subscribe-widget__button" value="<?php _e('Subscribe', 'wbcr_factory_templates_103'); ?>">
				</form>
			</div>
		</div>
	...
	{
		require_once FACTORY_TEMPLATES_103_DIR . '/includes/class-search-options.php';

		$options = $this->getPageOptions();
		$page_url = $this->getBaseUrl();
		$page_id = $this->getResultId();

		\WBCR\Factory_Templates_103\Search_Options::register_options($options, $page_url, $page_id);
	}
	...
	public function printAllNotices()
	{
		...
		?>
		<div id="wbcr-factory-templates-103__search_options_form" class="wbcr-factory-templates-103__autocomplete-wrap">
			<label for="autocomplete" class="wbcr-factory-templates-103__autocomplete-label">
				<?php _e('Can\'t find the settings you need? Use the search by the plugin options:', 'wbcr_factory_templates_103'); ?>
			</label>
			<input type="text" placeholder="<?php _e('Enter the option name to search...', 'wbcr_factory_templates_103'); ?>" name="country" id="wbcr-factory-templates-103__autocomplete"/>
